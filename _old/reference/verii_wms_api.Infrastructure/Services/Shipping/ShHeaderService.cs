...
namespace WMS_WEBAPI.Services
{
    public class ShHeaderService : IShHeaderService
    {
        ...
        public ShHeaderService(IUnitOfWork unitOfWork, IMapper mapper, ILocalizationService localizationService, ICurrentUserService executionContextAccessor, IErpService erpService, INotificationService notificationService, IRequestCancellationAccessor requestCancellationAccessor)
        {
...
var entities = await _unitOfWork.ShHeaders.Query().ToListAsync(requestCancellationToken);
...
return ApiResponse<PagedResponse<ShHeaderDto>>.SuccessResult(result, _localizationService.GetLocalizedString("ShHeaderRetrievedSuccessfully"));
        ...
        {
            var requestCancellationToken = ResolveCancellationToken(cancellationToken);
var entity = await _unitOfWork.ShHeaders.Query()
    .Where(x => x.Id == id)
...
return ApiResponse<ShHeaderDto>.SuccessResult(dto, _localizationService.GetLocalizedString("ShHeaderCreatedSuccessfully"));
        ...
        {
            var requestCancellationToken = ResolveCancellationToken(cancellationToken);
var existing = await _unitOfWork.ShHeaders.Query(tracking: true)
    .Where(x => x.Id == id)
    .FirstOrDefaultAsync(requestCancellationToken);
if (existing == null)
{
...
var importLines = await _unitOfWork.ShImportLines.Query().Where(x => x.HeaderId == id).ToListAsync(requestCancellationToken);
if (importLines.Any())
{
...
return ApiResponse<bool>.SuccessResult(true, _localizationService.GetLocalizedString("ShHeaderDeletedSuccessfully"));
        ...
        {
            var requestCancellationToken = ResolveCancellationToken(cancellationToken);
var entity = await _unitOfWork.ShHeaders.Query(tracking: true)
    .Where(x => x.Id == id)
    .FirstOrDefaultAsync(requestCancellationToken);
...
using (var tx = await _unitOfWork.BeginTransactionAsync())
{
    try
    {
        ...
        // 2. CREATE HEADER
        // ============================================
        var shHeader = _mapper.Map<ShHeader>(request.Header) ?? new ShHeader();
        ...
        if (request.ImportLines != null && request.ImportLines.Count > 0)
        {
            ...
            foreach (var importDto in request.ImportLines)
            {
                ...
                var importLine = _mapper.Map<ShImportLine>(importDto) ?? new ShImportLine();
        ...
        if (request.Routes != null && request.Routes.Count > 0)
        {
            ...
            foreach (var routeDto in request.Routes)
            {
                ...
                var route = _mapper.Map<ShRoute>(routeDto) ?? new ShRoute();
