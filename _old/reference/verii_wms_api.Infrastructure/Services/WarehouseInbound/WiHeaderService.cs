...
namespace WMS_WEBAPI.Services
{
    public class WiHeaderService : IWiHeaderService
    {
        ...
        public WiHeaderService(IUnitOfWork unitOfWork, IMapper mapper, ILocalizationService localizationService, ICurrentUserService executionContextAccessor, IErpService erpService, INotificationService notificationService, IRequestCancellationAccessor requestCancellationAccessor)
        {
...
return ApiResponse<WiHeaderDto>.SuccessResult(dto, _localizationService.GetLocalizedString("WiHeaderUpdatedSuccessfully"));
        ...
        {
            var requestCancellationToken = ResolveCancellationToken(cancellationToken);
var importLines = await _unitOfWork.WiImportLines.Query().Where(x => x.HeaderId == id).ToListAsync(requestCancellationToken);
if (importLines.Any())
{
    ...
    try
    {
        ...
        if (request.ImportLines != null && request.ImportLines.Count > 0)
        {
            ...
            foreach (var importDto in request.ImportLines)
            {
                ...
                var importLine = _mapper.Map<WiImportLine>(importDto) ?? new WiImportLine();
        ...
        if (request.Routes != null && request.Routes.Count > 0)
        {
            ...
            foreach (var routeDto in request.Routes)
            {
                ...
                var route = _mapper.Map<WiRoute>(routeDto) ?? new WiRoute();
