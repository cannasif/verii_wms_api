...
namespace WMS_WEBAPI.Controllers
{
    ...
    public class PrParameterController : ControllerBase
    {
        ...
        {
            var result = await _prParameterService.GetPagedAsync(request);
            return StatusCode(result.StatusCode, result);
        }

        [HttpPost("paged")]
        public async Task<ActionResult<ApiResponse<PagedResponse<PrParameterDto>>>> GetPaged([FromBody] PagedRequest request)
        {
            var result = await _prParameterService.GetPagedAsync(request);
            return StatusCode(result.StatusCode, result);
        }
