...
namespace WMS_WEBAPI.Controllers
{
    ...
    public class GrParameterController : ControllerBase
    {
        ...
        {
            var result = await _grParameterService.GetPagedAsync(request);
            return StatusCode(result.StatusCode, result);
        }

        [HttpPost("paged")]
        public async Task<ActionResult<ApiResponse<PagedResponse<GrParameterDto>>>> GetPaged([FromBody] PagedRequest request)
        {
            var result = await _grParameterService.GetPagedAsync(request);
            return StatusCode(result.StatusCode, result);
        }
