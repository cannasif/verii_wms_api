...
namespace WMS_WEBAPI.Controllers
{
    ...
    public class SitParameterController : ControllerBase
    {
        ...
        {
            var result = await _sitParameterService.GetPagedAsync(request);
            return StatusCode(result.StatusCode, result);
        }

        [HttpPost("paged")]
        public async Task<ActionResult<ApiResponse<PagedResponse<SitParameterDto>>>> GetPaged([FromBody] PagedRequest request)
        {
            var result = await _sitParameterService.GetPagedAsync(request);
            return StatusCode(result.StatusCode, result);
        }
