...
namespace WMS_WEBAPI.Controllers
{
    ...
    public class ShParameterController : ControllerBase
    {
        ...
        {
            var result = await _shParameterService.GetPagedAsync(request);
            return StatusCode(result.StatusCode, result);
        }

        [HttpPost("paged")]
        public async Task<ActionResult<ApiResponse<PagedResponse<ShParameterDto>>>> GetPaged([FromBody] PagedRequest request)
        {
            var result = await _shParameterService.GetPagedAsync(request);
            return StatusCode(result.StatusCode, result);
        }
