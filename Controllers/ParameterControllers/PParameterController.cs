...
namespace WMS_WEBAPI.Controllers
{
    ...
    public class PParameterController : ControllerBase
    {
        ...
        {
            var result = await _pParameterService.GetPagedAsync(request);
            return StatusCode(result.StatusCode, result);
        }

        [HttpPost("paged")]
        public async Task<ActionResult<ApiResponse<PagedResponse<PParameterDto>>>> GetPaged([FromBody] PagedRequest request)
        {
            var result = await _pParameterService.GetPagedAsync(request);
            return StatusCode(result.StatusCode, result);
        }
