...
namespace WMS_WEBAPI.Controllers
{
    ...
    public class UserAuthorityController : ControllerBase
    {
        ...
        {
            var result = await _userAuthorityService.GetPagedAsync(request);
            return StatusCode(result.StatusCode, result);
        }

        [HttpPost("paged")]
        public async Task<ActionResult<ApiResponse<PagedResponse<UserAuthorityDto>>>> GetPaged([FromBody] PagedRequest request)
        {
            var result = await _userAuthorityService.GetPagedAsync(request);
            return StatusCode(result.StatusCode, result);
        }
