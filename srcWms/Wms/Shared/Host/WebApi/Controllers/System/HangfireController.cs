...
public sealed class HangfireController : ControllerBase
{
    ...
    public HangfireController(IJobService jobs, IOptions<HangfireMonitoringOptions> options, WmsDbContext db) { _jobs = jobs; _options = options.Value; _db = db; }
    [HttpGet("stats")] public async Task<IActionResult> GetStats(CancellationToken cancellationToken = default) { var monitoringApi = JobStorage.Current.GetMonitoringApi(); return Ok(new { Enqueued = monitoringApi.EnqueuedCount("default"), Processing = monitoringApi.ProcessingCount(), Scheduled = monitoringApi.ScheduledCount(), Succeeded = monitoringApi.SucceededListCount(), Failed = await _db.JobFailureLogs.AsNoTracking().CountAsync(cancellationToken), Deleted = monitoringApi.DeletedListCount(), Servers = monitoringApi.Servers().Count, Queues = monitoringApi.Queues().Count, Timestamp = DateTime.UtcNow, Monitoring = new { _options.EnableFailureSqlLog, _options.FinalRetryCountThreshold, _options.CriticalJobs, _options.AlertEmails } }); }
