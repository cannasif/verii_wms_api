...
public sealed class WoHeaderController : ControllerBase
{
    ...
    [HttpPost("process")]
    public async Task<ActionResult<ApiResponse<int>>> Process([FromBody] ProcessWoRequestDto request, CancellationToken cancellationToken = default)
    {
        var result = await _service.ProcessWarehouseOutboundAsync(request, cancellationToken);
        return StatusCode(result.StatusCode, result);
    }
