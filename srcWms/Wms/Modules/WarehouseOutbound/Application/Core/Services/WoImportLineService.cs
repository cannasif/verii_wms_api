...
public sealed class WoImportLineService : IWoImportLineService
{
    ...
    public async Task<ApiResponse<WoImportLineDto?>> GetByIdAsync(long id, CancellationToken cancellationToken = default)
    {
        ...
        if (entity == null)
        {
            var msg = _localizationService.GetLocalizedString("WoImportLineNotFound");
    ...
    {
        var entity = _mapper.Map<WoImportLine>(createDto) ?? new WoImportLine();
