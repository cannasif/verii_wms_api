...
public sealed class WiRouteService : IWiRouteService
{
    ...
    public async Task<ApiResponse<IEnumerable<WiRouteDto>>> GetAllAsync(CancellationToken cancellationToken = default)
    {
        ...
        return ApiResponse<IEnumerable<WiRouteDto>>.SuccessResult(_mapper.Map<List<WiRouteDto>>(items), _localizationService.GetLocalizedString("WiRouteRetrievedSuccessfully"));
    }
    ...
    public async Task<ApiResponse<PagedResponse<WiRouteDto>>> GetPagedAsync(PagedRequest request, CancellationToken cancellationToken = default)
    {
        ...
        return ApiResponse<PagedResponse<WiRouteDto>>.SuccessResult(new PagedResponse<WiRouteDto>(_mapper.Map<List<WiRouteDto>>(items), total, request.PageNumber < 1 ? 1 : request.PageNumber, request.PageSize < 1 ? 20 : request.PageSize), _localizationService.GetLocalizedString("WiRouteRetrievedSuccessfully"));
    }
    ...
    public async Task<ApiResponse<WiRouteDto>> GetByIdAsync(long id, CancellationToken cancellationToken = default)
    {
        ...
        return ApiResponse<WiRouteDto>.SuccessResult(_mapper.Map<WiRouteDto>(entity), _localizationService.GetLocalizedString("WiRouteRetrievedSuccessfully"));
    }
