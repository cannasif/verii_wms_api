...
[Authorize]
public sealed class WtLineController : ControllerBase
{
    private readonly IWtLineService _service;

    public WtLineController(IWtLineService service)
    {
        _service = service;
    }
    ...
    public async Task<IActionResult> GetByHeaderId(long headerId, [FromBody] PagedRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var items = result.Data?.ToList() ?? new List<WtLineDto>();
        var pageNumber = request.PageNumber < 1 ? 1 : request.PageNumber;
        var pageSize = request.PageSize < 1 ? 20 : request.PageSize;
        var pagedData = items.Skip((pageNumber - 1) * pageSize).Take(pageSize).ToList();
        var pagedResult = result.Success
            ? ApiResponse<PagedResponse<WtLineDto>>.SuccessResult(new PagedResponse<WtLineDto>(pagedData, items.Count, pageNumber, pageSize), result.Message)
            : ApiResponse<PagedResponse<WtLineDto>>.ErrorResult(result.Message, result.ExceptionMessage, result.StatusCode);
