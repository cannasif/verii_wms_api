...
public sealed class PackageGoodsReceiptMatcher : IPackageGoodsReceiptMatcher
{
    ...
    public PackageGoodsReceiptMatcher(
    ...
    public async Task<ApiResponse<long>> MatchPackageLineToGoodsReceiptAsync(PHeader header, PLine packageLine, CancellationToken cancellationToken = default)
    {
        ...
        if (sourceHeader == null || sourceHeader.IsDeleted)
        {
            return Error<long>("SourceHeaderNotFound", 404);
        }
        ...
        if (hasSerialInLineSerials && hasRequestSerial)
        {
            ...
            var totalAfterAdd = totalRouteQuantity + packageLine.Quantity;
            var allowMore = parameter?.AllowMoreQuantityBasedOnOrder ?? false;
            if (!allowMore && totalAfterAdd > totalLineSerialQuantity + 0.000001m)
            {
                var localizedMessage = _localizationService.GetLocalizedString("PLineQuantityCannotBeGreater");
                var exceptionMessage = $"Serial {serialNo} (StockCode: {packageStockCode}, YapKod: {packageYapKod}): Route total after add ({totalAfterAdd}) cannot be greater than LineSerial total ({totalLineSerialQuantity})";
        ...
        else
        {
            ...
            {
                var localizedMessage = _localizationService.GetLocalizedString("PLineQuantityCannotBeGreater");
                var exceptionMessage = $"StockCode: {packageStockCode}, YapKod: {packageYapKod}: Route total after add ({totalAfterAdd}) cannot be greater than LineSerial total ({totalLineSerialQuantity})";
                return ApiResponse<long>.ErrorResult(localizedMessage, exceptionMessage, 400);
            }
        }
        ...
        if (!selectedLineId.HasValue)
        {
            return Error<long>("PLineNoMatchingLine", 400);
        }
        ...
        {
            existingImportLine = new GrImportLine
            {
