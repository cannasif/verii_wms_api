...
public sealed class GrLineSerialConfiguration : BaseLineSerialEntityConfiguration<GrLineSerial>
{
    protected override void ConfigureEntity(EntityTypeBuilder<GrLineSerial> builder)
    {
        ...
        builder.HasOne(x => x.Line).WithMany(x => x.SerialLines).HasForeignKey(x => x.LineId).OnDelete(DeleteBehavior.Restrict).HasConstraintName("FK_GrLineSerial_GrLine");
        builder.HasIndex(x => x.LineId).HasDatabaseName("IX_GrLineSerial_LineId");
