using AutoMapper;
using Wms.Application.Common;
using Wms.Application.Definitions.Dtos.Parameters;
using Wms.Domain.Entities.Definitions;

namespace Wms.Application.Definitions.Services.Parameters;

public sealed class WoParameterService
    : ParameterCrudService<WoParameter, WoParameterDto, CreateWoParameterDto, UpdateWoParameterDto>,
      IWoParameterService
{
    public WoParameterService(
        IRepository<WoParameter> repository,
        IUnitOfWork unitOfWork,
        IMapper mapper,
        ILocalizationService localizationService)
        : base(repository, unitOfWork, mapper, localizationService)
    {
    }
}
