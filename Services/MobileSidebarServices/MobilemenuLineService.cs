...
namespace WMS_WEBAPI.Services
{
    public class MobilemenuLineService : IMobilemenuLineService
    {
        ...
        public async Task<ApiResponse<PagedResponse<MobilemenuLineDto>>> GetPagedAsync(PagedRequest request)
        {
            try
            {
                ...
                var query = _unitOfWork.MobilemenuLines.AsQueryable().Where(x => !x.IsDeleted);
                ...
                var dtos = _mapper.Map<List<MobilemenuLineDto>>(entities);
                var result = new PagedResponse<MobilemenuLineDto>(dtos, totalCount, request.PageNumber, request.PageSize);

                return ApiResponse<PagedResponse<MobilemenuLineDto>>.SuccessResult(
                    result,
                    _localizationService.GetLocalizedString("MobilemenuLineRetrievedSuccessfully"));
            }
            catch (Exception ex)
            {
                return ApiResponse<PagedResponse<MobilemenuLineDto>>.ErrorResult(
                    _localizationService.GetLocalizedString("MobilemenuLineErrorOccurred"),
