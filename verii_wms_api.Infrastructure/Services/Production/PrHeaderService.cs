...
namespace WMS_WEBAPI.Services
{
    public class PrHeaderService : IPrHeaderService
    {
        ...
        public async Task<ApiResponse<PrHeaderDto>> GetByIdAsync(long id)
        {
            try
            {
                var entity = await _unitOfWork.PrHeaders.Query()
                    .Where(x => x.Id == id)
                    .FirstOrDefaultAsync();
        ...
        public async Task<ApiResponse<bool>> CompleteAsync(long id)
        {
            try
            {
                ...
                if (!skipValidation)
                {
                    var lines = await _unitOfWork.PrLines.Query()
                    ...
                    foreach (var line in lines)
                    {
                        ...
                        var totalLineSerialQuantity = await _unitOfWork.PrLineSerials.Query()
            ...
            try
            {
                using (var tx = await _unitOfWork.BeginTransactionAsync())
                {
                        ...
                        if (request.Lines != null && request.Lines.Count > 0)
                        {
                            ...
                            {
                            var line = _mapper.Map<PrLine>(l) ?? new PrLine();
                        ...
                        if (request.HeaderSerials != null && request.HeaderSerials.Count > 0)
                        {
                            ...
                            {
                            var hSerial = _mapper.Map<PrHeaderSerial>(hs) ?? new PrHeaderSerial();
            ...
            try
            {
                using (var tx = await _unitOfWork.BeginTransactionAsync())
                {
                    try
                    {
                        ...
                        // 2. CREATE HEADER
                        // ============================================
                        var headerEntity = _mapper.Map<PrHeader>(request.Header.Header) ?? new PrHeader();
                        ...
                        if (request.HeaderSerials != null && request.HeaderSerials.Count > 0)
                        {
                            ...
                            foreach (var headerSerialDto in request.HeaderSerials)
                            {
                                ...
                                var hSerial = _mapper.Map<PrHeaderSerial>(headerSerialDto) ?? new PrHeaderSerial();
                        ...
                        if (request.ImportLines != null && request.ImportLines.Count > 0)
                        {
                            ...
                            foreach (var importDto in request.ImportLines)
                            {
                                ...
                                var importLine = _mapper.Map<PrImportLine>(importDto) ?? new PrImportLine();
        ...
        public async Task<ApiResponse<IEnumerable<PrHeaderDto>>> GetAssignedProductionOrdersAsync(long userId)
        {
            try
            {
                ...
                var query = _unitOfWork.PrHeaders.Query()
                    .Where(h => !h.IsDeleted 
                        && !h.IsCompleted 
                        && h.BranchCode == branchCode
                        && _unitOfWork.PrTerminalLines.Query(false, false)
