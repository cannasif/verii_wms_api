...
return ApiResponse<IEnumerable<PrImportLineDto>>.SuccessResult(enriched.Data ?? dtos, _localizationService.GetLocalizedString("PrImportLineRetrievedSuccessfully"));
        ...
        public async Task<ApiResponse<PagedResponse<PrImportLineDto>>> GetPagedAsync(PagedRequest request)
        {
...
{
    return ApiResponse<PagedResponse<PrImportLineDto>>.ErrorResult(
        allResult.Message,
...
var result = new PagedResponse<PrImportLineDto>(items, totalCount, request.PageNumber, request.PageSize);
return ApiResponse<PagedResponse<PrImportLineDto>>.SuccessResult(result, allResult.Message);
        }


        public async Task<ApiResponse<PrImportLineDto>> GetByIdAsync(long id)
        {
var entity = await _unitOfWork.PrImportLines.Query()
    .Where(x => x.Id == id)
    .FirstOrDefaultAsync();
...
var importLines = await _unitOfWork.PrImportLines.Query()
...
var routes = await _unitOfWork.PrRoutes.Query()
...
using (var tx = await _unitOfWork.BeginTransactionAsync())
{
    try
    {
        ...
        var matchingLines = await _unitOfWork.PrLines.Query()
        ...
        var lineSerials = await _unitOfWork.PrLineSerials.Query()
        ...
        var prParameter = await _unitOfWork.PrParameters.Query()
            .Where(p => !p.IsDeleted)
            .FirstOrDefaultAsync();

        {
            ...
            if (hasSerialInLineSerials && hasRequestSerial)
            {
                ...
                var totalRouteQuantity = await _unitOfWork.PrRoutes.Query()
                    .Where(r => !r.IsDeleted
        ...
        if (!selectedLineId.HasValue)
        {
            ...
            foreach (var line in matchingLines)
            {
                ...
                var routeTotal = await _unitOfWork.PrRoutes.Query()
                    .Where(r => !r.IsDeleted
        ...
        PrImportLine? importLine = await _unitOfWork.PrImportLines.Query()
