...
namespace WMS_WEBAPI.Services
{
    public class PtHeaderService : IPtHeaderService
    {
        ...
        public PtHeaderService(IUnitOfWork unitOfWork, IMapper mapper, ILocalizationService localizationService, IExecutionContextAccessor executionContextAccessor, IErpService erpService, INotificationService notificationService)
        {
...
var entities = await _unitOfWork.PtHeaders.Query().Where(x => x.BranchCode == branchCode).ToListAsync();
...
var branchCode = _executionContextAccessor.BranchCode ?? "0";
var query = _unitOfWork.PtHeaders.Query().Where(x => x.BranchCode == branchCode);
...
return ApiResponse<PtHeaderDto>.SuccessResult(finalDto, _localizationService.GetLocalizedString("PtHeaderRetrievedSuccessfully"));
        ...
        {
var branchCode = _executionContextAccessor.BranchCode ?? "0";
var entities = await _unitOfWork.PtHeaders.Query().Where(x => x.CustomerCode == customerCode && x.BranchCode == branchCode).ToListAsync();
var dtos = _mapper.Map<IEnumerable<PtHeaderDto>>(entities);
...
return ApiResponse<IEnumerable<PtHeaderDto>>.SuccessResult(dtos, _localizationService.GetLocalizedString("PtHeaderRetrievedSuccessfully"));
        ...
        {
var entities = await _unitOfWork.PtHeaders.Query().Where(x => x.DocumentType == documentType).ToListAsync();
var dtos = _mapper.Map<IEnumerable<PtHeaderDto>>(entities);
...
var importLines = await _unitOfWork.PtImportLines.Query().Where(x => x.HeaderId == id).ToListAsync();
if (importLines.Any())
{
...
return ApiResponse<IEnumerable<PtHeaderDto>>.SuccessResult(dtos, _localizationService.GetLocalizedString("PtHeaderAssignedOrdersRetrievedSuccessfully"));
        ...
        {
var lines = await _unitOfWork.PtLines.Query().Where(x => x.HeaderId == headerId).ToListAsync();
...
using (var tx = await _unitOfWork.BeginTransactionAsync())
{
    try
    {
        ...
        // 2. CREATE HEADER
        // ============================================
        var headerEntity = _mapper.Map<PtHeader>(request.Header.Header) ?? new PtHeader();
        ...
        var headerRequest = request.Header!;
        var headerKeyToId = new Dictionary<Guid, long> { { headerRequest.HeaderKey, headerEntity.Id } };
        ...
        if (request.ImportLines != null && request.ImportLines.Count > 0)
        {
            ...
            foreach (var importDto in request.ImportLines)
            {
                ...
                var importLine = _mapper.Map<PtImportLine>(importDto) ?? new PtImportLine();
