...
namespace WMS_WEBAPI.Services
{
    public class PtImportLineService : IPtImportLineService
    {
        ...
        public async Task<ApiResponse<IEnumerable<PtImportLineDto>>> GetAllAsync()
        {
            try
            {
                var entities = await _unitOfWork.PtImportLines.Query().ToListAsync();
        ...
        public async Task<ApiResponse<PagedResponse<PtImportLineDto>>> GetPagedAsync(PagedRequest request)
        {
            try
            {
                ...
                {
                    return ApiResponse<PagedResponse<PtImportLineDto>>.ErrorResult(
                        allResult.Message,
                ...
                var result = new PagedResponse<PtImportLineDto>(items, totalCount, request.PageNumber, request.PageSize);
                return ApiResponse<PagedResponse<PtImportLineDto>>.SuccessResult(result, allResult.Message);
            }
            catch (Exception ex)
            {
                return ApiResponse<PagedResponse<PtImportLineDto>>.ErrorResult(
        ...
        public async Task<ApiResponse<PtImportLineDto>> GetByIdAsync(long id)
        {
            try
            {
                var entity = await _unitOfWork.PtImportLines.Query()
                    .Where(x => x.Id == id)
                    .FirstOrDefaultAsync();
        ...
        public async Task<ApiResponse<IEnumerable<PtImportLineDto>>> GetByHeaderIdAsync(long headerId)
        {
            try
            {
                var entities = await _unitOfWork.PtImportLines.Query().Where(x => x.HeaderId == headerId).ToListAsync();
                var dtos = _mapper.Map<IEnumerable<PtImportLineDto>>(entities);
        ...
        public async Task<ApiResponse<IEnumerable<PtImportLineDto>>> GetByLineIdAsync(long lineId)
        {
            try
            {
                var entities = await _unitOfWork.PtImportLines.Query().Where(x => x.LineId == lineId).ToListAsync();
                var dtos = _mapper.Map<IEnumerable<PtImportLineDto>>(entities);
        ...
        public async Task<ApiResponse<bool>> SoftDeleteAsync(long id)
        {
            try
            {
                ...
                var routes = await _unitOfWork.PtRoutes.Query().Where(x => x.ImportLineId == id).ToListAsync();
                if (routes.Any())
                {
                    var msg = _localizationService.GetLocalizedString("PtImportLineRoutesExist");
                    return ApiResponse<bool>.ErrorResult(msg, msg, 400);
                }
                var hasActiveLineSerials = await _unitOfWork.PtLineSerials.Query()
                ...
                try
                {
                    ...
                    var hasOtherLines = await _unitOfWork.PtLines.Query()
                        .Where(l => !l.IsDeleted && l.HeaderId == headerId)
                            .AnyAsync();
                    var hasOtherImportLines = await _unitOfWork.PtImportLines.Query()
        ...
        public async Task<ApiResponse<IEnumerable<PtImportLineWithRoutesDto>>> GetCollectedBarcodesByHeaderIdAsync(long headerId)
        {
            try
            {
                ...
                var importLines = await _unitOfWork.PtImportLines.Query()
                ...
                var routes = await _unitOfWork.PtRoutes.Query()
            ...
            try
            {
                using (var tx = await _unitOfWork.BeginTransactionAsync())
                {
                    try
                    {
                        ...
                        var matchingLines = await _unitOfWork.PtLines.Query()
                        ...
                        var lineSerials = await _unitOfWork.PtLineSerials.Query()
                        ...
                        if (!selectedLineId.HasValue)
                        {
                            ...
                            foreach (var line in matchingLines)
                            {
                                ...
                                var routeTotal = await _unitOfWork.PtRoutes.Query()
                                    .Where(r => !r.IsDeleted
                        ...
                        PtImportLine? importLine = await _unitOfWork.PtImportLines.Query()
