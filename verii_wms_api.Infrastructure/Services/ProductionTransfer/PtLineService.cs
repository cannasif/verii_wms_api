    ...
    public class PtLineService : IPtLineService
    {
        ...
        public async Task<ApiResponse<PagedResponse<PtLineDto>>> GetPagedAsync(PagedRequest request)
        {
            try
            {
                ...
                var query = _unitOfWork.PtLines.Query();
        ...
        public async Task<ApiResponse<IEnumerable<PtLineDto>>> GetAllAsync()
        {
            try
            {
                var entities = await _unitOfWork.PtLines.Query().ToListAsync();
        ...
        public async Task<ApiResponse<PtLineDto>> GetByIdAsync(long id)
        {
            try
            {
                var entity = await _unitOfWork.PtLines.Query()
                    .Where(x => x.Id == id)
                    .FirstOrDefaultAsync();
        ...
        public async Task<ApiResponse<IEnumerable<PtLineDto>>> GetByHeaderIdAsync(long headerId)
        {
            try
            {
                var entities = await _unitOfWork.PtLines.Query().Where(x => x.HeaderId == headerId).ToListAsync();
                var dtos = _mapper.Map<IEnumerable<PtLineDto>>(entities);
                var enriched = await _erpService.PopulateStockNamesAsync(dtos);
                if (!enriched.Success)
                {
        ...
        public async Task<ApiResponse<bool>> SoftDeleteAsync(long id)
        {
            try
            {
                ...
                var importLines = await _unitOfWork.PtImportLines.Query().Where(x => x.LineId == id).ToListAsync();
                if (importLines.Any())
                {
