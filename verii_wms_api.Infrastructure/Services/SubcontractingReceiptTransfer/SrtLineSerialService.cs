    ...
    public class SrtLineSerialService : ISrtLineSerialService
    {
        ...
        public async Task<ApiResponse<PagedResponse<SrtLineSerialDto>>> GetPagedAsync(PagedRequest request, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                {
                    return ApiResponse<PagedResponse<SrtLineSerialDto>>.ErrorResult(
                        allResult.Message,
                ...
                var result = new PagedResponse<SrtLineSerialDto>(items, totalCount, request.PageNumber, request.PageSize);
                return ApiResponse<PagedResponse<SrtLineSerialDto>>.SuccessResult(result, allResult.Message);
            }
            catch (Exception ex)
            {
                return ApiResponse<PagedResponse<SrtLineSerialDto>>.ErrorResult(
        ...
        public async Task<ApiResponse<bool>> SoftDeleteAsync(long id, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
...

                {
                    ...
                    if (anyEntitySerial)
                    {
                        var serialExistsInRoutes = await _unitOfWork.SrtRoutes.Query()
                            .Where(r => !r.IsDeleted
                ...
                var totalLineSerialQty = await _unitOfWork.SrtLineSerials.Query()
                    .Where(ls => !ls.IsDeleted && ls.LineId == entity.LineId)
                    .SumAsync(ls => ls.Quantity);

                var totalRouteQty = await _unitOfWork.SrtRoutes.Query()
