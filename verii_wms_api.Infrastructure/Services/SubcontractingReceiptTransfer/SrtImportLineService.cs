...
{
    return ApiResponse<PagedResponse<SrtImportLineDto>>.ErrorResult(
        allResult.Message,
...
var result = new PagedResponse<SrtImportLineDto>(items, totalCount, request.PageNumber, request.PageSize);
return ApiResponse<PagedResponse<SrtImportLineDto>>.SuccessResult(result, allResult.Message);
        }
...
using (var tx = await _unitOfWork.BeginTransactionAsync())
{
    try
    {
        ...
        var matchingLines = await _unitOfWork.SrtLines.Query()
...

        {
            ...
            if (hasSerialInLineSerials && hasRequestSerial)
            {
                ...
                var totalRouteQuantity = await _unitOfWork.SrtRoutes.Query()
                    .Where(r => !r.IsDeleted
        ...
        if (!selectedLineId.HasValue)
        {
            ...
            foreach (var line in matchingLines)
            {
                ...
                var routeTotal = await _unitOfWork.SrtRoutes.Query()
                    .Where(r => !r.IsDeleted
        ...
        SrtImportLine? importLine = await _unitOfWork.SrtImportLines.Query()
