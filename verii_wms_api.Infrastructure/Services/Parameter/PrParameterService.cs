...
namespace WMS_WEBAPI.Services
{
    public class PrParameterService : IPrParameterService
    {
        ...
        public PrParameterService(IUnitOfWork unitOfWork, IMapper mapper, ILocalizationService localizationService, IRequestCancellationAccessor requestCancellationAccessor)
        {
        ...
        public async Task<ApiResponse<IEnumerable<PrParameterDto>>> GetAllAsync(CancellationToken cancellationToken = default)
        {
            try
            {
                var requestCancellationToken = ResolveCancellationToken(cancellationToken);
                var entities = await _unitOfWork.PrParameters.GetAllAsync(requestCancellationToken);
        ...
        public async Task<ApiResponse<PagedResponse<PrParameterDto>>> GetPagedAsync(PagedRequest request, CancellationToken cancellationToken = default)
        {
            try
            {
                var requestCancellationToken = ResolveCancellationToken(cancellationToken);
                ...
                var query = _unitOfWork.PrParameters.Query();
                ...
                var dtos = _mapper.Map<List<PrParameterDto>>(entities);
                var result = new PagedResponse<PrParameterDto>(dtos, totalCount, request.PageNumber, request.PageSize);

                return ApiResponse<PagedResponse<PrParameterDto>>.SuccessResult(
                    result,
                    _localizationService.GetLocalizedString("ParameterRetrievedSuccessfully"));
            }
            catch (Exception ex)
            {
                return ApiResponse<PagedResponse<PrParameterDto>>.ErrorResult(
        ...
        public async Task<ApiResponse<PrParameterDto>> GetByIdAsync(long id, CancellationToken cancellationToken = default)
        {
            try
            {
                var requestCancellationToken = ResolveCancellationToken(cancellationToken);
                var entity = await _unitOfWork.PrParameters.Query()
                    .Where(x => x.Id == id)
        ...
        public async Task<ApiResponse<PrParameterDto>> CreateAsync(CreatePrParameterDto createDto, CancellationToken cancellationToken = default)
        {
            try
            {
                var requestCancellationToken = ResolveCancellationToken(cancellationToken);
                var entity = _mapper.Map<PrParameter>(createDto);
                await _unitOfWork.PrParameters.AddAsync(entity, requestCancellationToken);
        ...
        public async Task<ApiResponse<PrParameterDto>> UpdateAsync(long id, UpdatePrParameterDto updateDto, CancellationToken cancellationToken = default)
        {
            try
            {
                var requestCancellationToken = ResolveCancellationToken(cancellationToken);
        ...
        public async Task<ApiResponse<bool>> SoftDeleteAsync(long id, CancellationToken cancellationToken = default)
        {
            ...
            {
                var requestCancellationToken = ResolveCancellationToken(cancellationToken);
                var exists = await _unitOfWork.PrParameters.ExistsAsync(id, requestCancellationToken);
                if (!exists)
                {
                ...
                await _unitOfWork.PrParameters.SoftDelete(id, requestCancellationToken);
                await _unitOfWork.SaveChangesAsync(requestCancellationToken);
