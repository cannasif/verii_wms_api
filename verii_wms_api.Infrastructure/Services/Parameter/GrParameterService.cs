...
namespace WMS_WEBAPI.Services
{
    public class GrParameterService : IGrParameterService
    {
        ...
        public async Task<ApiResponse<PagedResponse<GrParameterDto>>> GetPagedAsync(PagedRequest request)
        {
            try
            {
                ...
                var query = _unitOfWork.GrParameters.AsQueryable().Where(x => !x.IsDeleted);
