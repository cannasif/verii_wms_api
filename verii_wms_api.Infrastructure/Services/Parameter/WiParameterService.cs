...
namespace WMS_WEBAPI.Services
{
    public class WiParameterService : IWiParameterService
    {
        ...
        public async Task<ApiResponse<PagedResponse<WiParameterDto>>> GetPagedAsync(PagedRequest request)
        {
            try
            {
                ...
                var query = _unitOfWork.WiParameters.AsQueryable().Where(x => !x.IsDeleted);
