...
namespace WMS_WEBAPI.Services
{
    public class SitParameterService : ISitParameterService
    {
        ...
        public SitParameterService(IUnitOfWork unitOfWork, IMapper mapper, ILocalizationService localizationService)
        {
        ...
        public async Task<ApiResponse<IEnumerable<SitParameterDto>>> GetAllAsync()
        {
            try
            {
                var entities = await _unitOfWork.SitParameters.GetAllAsync();
            ...
            {
                return ApiResponse<IEnumerable<SitParameterDto>>.ErrorResult(_localizationService.GetLocalizedString("Error_GetAll"), ex.Message ?? string.Empty, 500);
            }
        }

        public async Task<ApiResponse<PagedResponse<SitParameterDto>>> GetPagedAsync(PagedRequest request)
        {
            try
            {
                ...
                var query = _unitOfWork.SitParameters.AsQueryable().Where(x => !x.IsDeleted);
            ...
            {
                return ApiResponse<PagedResponse<SitParameterDto>>.ErrorResult(
        ...
        public async Task<ApiResponse<SitParameterDto>> GetByIdAsync(long id)
        {
            try
            {
                var entity = await _unitOfWork.SitParameters.GetByIdAsync(id);
                if (entity == null)
                {
            ...
            {
                return ApiResponse<SitParameterDto>.ErrorResult(_localizationService.GetLocalizedString("Error_GetById"), ex.Message ?? string.Empty, 500);
            }
        }

        public async Task<ApiResponse<SitParameterDto>> CreateAsync(CreateSitParameterDto createDto)
        {
            try
            {
                var entity = _mapper.Map<SitParameter>(createDto);
                await _unitOfWork.SitParameters.AddAsync(entity);
            ...
            {
                return ApiResponse<SitParameterDto>.ErrorResult(_localizationService.GetLocalizedString("Error_Create"), ex.Message ?? string.Empty, 500);
            }
        }

        public async Task<ApiResponse<SitParameterDto>> UpdateAsync(long id, UpdateSitParameterDto updateDto)
        {
            try
            {
                var entity = await _unitOfWork.SitParameters.GetByIdAsync(id);
            ...
            {
                return ApiResponse<SitParameterDto>.ErrorResult(_localizationService.GetLocalizedString("Error_Update"), ex.Message ?? string.Empty, 500);
            }
        }
        ...
        public async Task<ApiResponse<bool>> SoftDeleteAsync(long id)
        {
            try
            {
                var exists = await _unitOfWork.SitParameters.ExistsAsync(id);
                if (!exists)
                {
                ...
                await _unitOfWork.SitParameters.SoftDelete(id);
                await _unitOfWork.SaveChangesAsync();
