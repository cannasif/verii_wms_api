...
namespace WMS_WEBAPI.Services
{
    public class WoRouteService : IWoRouteService
    {
        ...
        public async Task<ApiResponse<PagedResponse<WoRouteDto>>> GetPagedAsync(PagedRequest request)
        {
            try
            {
                ...
                var query = _unitOfWork.WoRoutes.AsQueryable().Where(x => !x.IsDeleted);
            ...
            {
                return ApiResponse<PagedResponse<WoRouteDto>>.ErrorResult(
                    _localizationService.GetLocalizedString("WoRouteRetrievalError"),
