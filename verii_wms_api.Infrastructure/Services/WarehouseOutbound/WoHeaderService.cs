...
namespace WMS_WEBAPI.Services
{
    public class WoHeaderService : IWoHeaderService
    {
        ...
        public WoHeaderService(IUnitOfWork unitOfWork, IMapper mapper, ILocalizationService localizationService, IExecutionContextAccessor executionContextAccessor, IErpService erpService, INotificationService notificationService, IRequestCancellationAccessor requestCancellationAccessor)
        {
...
var entities = await _unitOfWork.WoHeaders.Query().Where(x => x.BranchCode == branchCode).ToListAsync(requestCancellationToken);
...
var branchCode = _executionContextAccessor.BranchCode ?? "0";
var query = _unitOfWork.WoHeaders.Query()
...
return ApiResponse<WoHeaderDto>.SuccessResult(dto, _localizationService.GetLocalizedString("WoHeaderUpdatedSuccessfully"));
        ...
        {
            var requestCancellationToken = ResolveCancellationToken(cancellationToken);
var importLines = await _unitOfWork.WoImportLines.Query().Where(x => x.HeaderId == id).ToListAsync(requestCancellationToken);
if (importLines.Any())
{
    ...
    try
    {
        ...
        if (request.ImportLines != null && request.ImportLines.Count > 0)
        {
            ...
            foreach (var importDto in request.ImportLines)
            {
                ...
                var importLine = _mapper.Map<WoImportLine>(importDto) ?? new WoImportLine();
        ...
        if (request.Routes != null && request.Routes.Count > 0)
        {
            ...
            foreach (var routeDto in request.Routes)
            {
                ...
                var route = _mapper.Map<WoRoute>(routeDto) ?? new WoRoute();
