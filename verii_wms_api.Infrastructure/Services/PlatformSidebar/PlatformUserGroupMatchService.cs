...
namespace WMS_WEBAPI.Services
{
    public class PlatformUserGroupMatchService : IPlatformUserGroupMatchService
    {
        ...
        public async Task<ApiResponse<PagedResponse<PlatformUserGroupMatchDto>>> GetPagedAsync(PagedRequest request)
        {
            try
            {
                ...
                var query = _unitOfWork.PlatformUserGroupMatches.AsQueryable().Where(x => !x.IsDeleted);
                ...
                var dtos = _mapper.Map<List<PlatformUserGroupMatchDto>>(entities);
                var result = new PagedResponse<PlatformUserGroupMatchDto>(dtos, totalCount, request.PageNumber, request.PageSize);

                return ApiResponse<PagedResponse<PlatformUserGroupMatchDto>>.SuccessResult(
                    result,
                    _localizationService.GetLocalizedString("PlatformUserGroupMatchRetrievedSuccessfully"));
            }
            catch (Exception ex)
            {
                return ApiResponse<PagedResponse<PlatformUserGroupMatchDto>>.ErrorResult(
