    ...
    public class ShLineSerialService : IShLineSerialService
    {
        ...
        public async Task<ApiResponse<PagedResponse<ShLineSerialDto>>> GetPagedAsync(PagedRequest request)
        {
            try
            {
                ...
                var query = _unitOfWork.ShLineSerials.Query();
                ...
                return ApiResponse<PagedResponse<ShLineSerialDto>>.SuccessResult(
                    result,
                    _localizationService.GetLocalizedString("ShLineSerialRetrievedSuccessfully"));
            }
            catch (Exception ex)
            {
                return ApiResponse<PagedResponse<ShLineSerialDto>>.ErrorResult(
                    _localizationService.GetLocalizedString("ShLineSerialErrorOccurred"),
