...
namespace WMS_WEBAPI.Services
{
    public class ShImportLineService : IShImportLineService
    {
        ...
        public async Task<ApiResponse<PagedResponse<ShImportLineDto>>> GetPagedAsync(PagedRequest request)
        {
            try
            {
        ...
        public async Task<ApiResponse<bool>> SoftDeleteAsync(long id)
        {
            try
            {
                ...
                var routes = await _unitOfWork.ShRoutes.Query().Where(x => x.ImportLineId == id).ToListAsync();
                if (routes.Any())
                {
