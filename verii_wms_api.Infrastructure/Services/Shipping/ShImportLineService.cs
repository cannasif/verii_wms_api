...
namespace WMS_WEBAPI.Services
{
    public class ShImportLineService : IShImportLineService
    {
        ...
        public ShImportLineService(IUnitOfWork unitOfWork, IMapper mapper, ILocalizationService localizationService, IErpService erpService, IRequestCancellationAccessor requestCancellationAccessor)
        {
        ...
        public async Task<ApiResponse<PagedResponse<ShImportLineDto>>> GetPagedAsync(PagedRequest request, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var query = _unitOfWork.ShImportLines.Query();
                ...
                var dtos = _mapper.Map<List<ShImportLineDto>>(entities);
                var result = new PagedResponse<ShImportLineDto>(dtos, totalCount, request.PageNumber, request.PageSize);

                return ApiResponse<PagedResponse<ShImportLineDto>>.SuccessResult(
                    result,
                    _localizationService.GetLocalizedString("ShImportLineRetrievedSuccessfully"));
            }
            catch (Exception ex)
            {
                return ApiResponse<PagedResponse<ShImportLineDto>>.ErrorResult(
                    _localizationService.GetLocalizedString("ShImportLineErrorOccurred"),
        ...
        public async Task<ApiResponse<ShImportLineDto>> GetByIdAsync(long id, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                var entity = await _unitOfWork.ShImportLines.Query()
                    .Where(x => x.Id == id)
        ...
        public async Task<ApiResponse<ShImportLineDto>> UpdateAsync(long id, UpdateShImportLineDto updateDto, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                var existing = await _unitOfWork.ShImportLines.Query()
                    .Where(x => x.Id == id)
                    .FirstOrDefaultAsync(requestCancellationToken);
                if (existing == null)
                {
        ...
        public async Task<ApiResponse<bool>> SoftDeleteAsync(long id, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var routes = await _unitOfWork.ShRoutes.Query().Where(x => x.ImportLineId == id).ToListAsync(requestCancellationToken);
                if (routes.Any())
                {
